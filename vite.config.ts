  ...
  return defineConfig({
    ...
    plugins: [
      ...
      UniComponents({
        ...
        dts: false, // 沙箱环境禁止改写已有文件，关闭 d.ts 自动生成（模板已自带类型）
      }),
      UniPages({
        ...
        dts: false, // 沙箱环境禁止改写已有文件，关闭 d.ts 自动生成
      }),
      ...
      AutoImport({
        imports: ['vue', 'uni-app'],
        dts: false, // 沙箱环境禁止改写已有文件，模板已自带 src/types/auto-import.d.ts
        dirs: ['src/hooks', 'src/store'], // 自动导入 hooks 与 store
    ...
    css: {
      preprocessorOptions: {
        scss: {
          // 官方推荐：使用 modern-compiler API 并静默 legacy-js-api 废弃警告
          // @see https://wot-ui.cn/guide/quick-use.html#sass
          api: 'modern-compiler',
