...
<template>
  <view class="flex flex-col items-center justify-center">
    ...
    <view v-if="!isBulge" class="mt-2px text-12px" :class="tabbarStore.curIdx === index ? 'font-medium' : ''">
      {{ item.text }}
    </view>
    <!-- 角标显示 -->
    <view v-if="item.badge">
      <template v-if="item.badge === 'dot'">
        <view class="absolute right-0 top-0 h-2 w-2 rounded-full bg-red-500" />
      </template>
      <template v-else>
        <view class="absolute top-0 box-border h-5 min-w-5 center rounded-full bg-red-500 px-1 text-center text-xs text-white -right-3">
