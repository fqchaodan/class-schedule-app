...
// 写入失败不阻塞启动（例如文件被其他进程占用时，保留现有内容即可）
function safeWriteIfEmpty(file, content) {
  try {
    if (!fs.existsSync(file) || fs.statSync(file).size <= MIN_SIZE)
      fs.writeFileSync(file, content)
  }
  catch (e) {
    console.warn(`[init-baseFiles] 跳过写入 ${file}: ${e.message}`)
  }
}

// 如果 src/manifest.json 不存在，就创建它；或者如果文件大小小于等于 MIN_SIZE，也重新创建
safeWriteIfEmpty(manifestPath, JSON.stringify(manifest, null, 2))

// 如果 src/pages.json 不存在，就创建它；或者如果文件大小小于等于 MIN_SIZE，也重新创建
safeWriteIfEmpty(pagesPath, JSON.stringify(pages, null, 2))
